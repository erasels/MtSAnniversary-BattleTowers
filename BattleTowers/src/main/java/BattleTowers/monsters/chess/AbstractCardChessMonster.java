package BattleTowers.monsters.chess;
...
import static BattleTowers.monsters.chess.AdditionalIntent.ENEMY_CARD_HOVERED_HEIGHT;
import static BattleTowers.monsters.chess.AdditionalIntent.ENEMY_CARD_HOVERED_WIDTH;

public abstract class AbstractCardChessMonster extends AbstractMultiIntentMonster {
    ...
    public AbstractCardChessMonster(String name, String id, int maxHealth, float hb_x, float hb_y, float hb_w, float hb_h, String imgUrl, float offsetX, float offsetY) {
        super(name, id, maxHealth, hb_x, hb_y, hb_w, hb_h, imgUrl, offsetX, offsetY);
    }

    public AbstractCardChessMonster(String name, String id, int maxHealth, float hb_x, float hb_y, float hb_w, float hb_h, String imgUrl, float offsetX, float offsetY, boolean ignoreBlights) {
        super(name, id, maxHealth, hb_x, hb_y, hb_w, hb_h, imgUrl, offsetX, offsetY, ignoreBlights);
    }

    public AbstractCardChessMonster(String name, String id, int maxHealth, float hb_x, float hb_y, float hb_w, float hb_h, String imgUrl) {
    ...
    public void update() {
        ...
        if (enemyCard != null && !this.isDeadOrEscaped() && !hasPower(StunMonsterPower.POWER_ID)) {
            ...
            if (enemyCard.hb.hovered) {
                if (AbstractCardChessMonster.hoveredCard == null) {
                    AbstractCardChessMonster.hoveredCard = enemyCard;
                }
            } else {
                if (AbstractCardChessMonster.hoveredCard == enemyCard) {
                    AbstractCardChessMonster.hoveredCard = null;
                }
            }
            if (AbstractCardChessMonster.hoveredCard == enemyCard) {
    ...
    public void rollMove() {
        cardsToRender.clear();
        super.rollMove();
        AbstractCardChessMonster.hoveredCard = null; //in case the player was hovering one of them while it was getting yeeted
    }
