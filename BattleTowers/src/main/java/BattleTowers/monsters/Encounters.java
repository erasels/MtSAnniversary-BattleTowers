package BattleTowers.monsters;

import static BattleTowers.BattleTowers.makeID;

public class Encounters {
    public static final String DBZ_PUNS = BattleTowers.makeID("Cawcawrot_Veggieta");
    public static final String MINOTAUR_GLADIATOR_AND_FRIEND = makeID("MINOTAUR_GLADIATOR_AND_FRIEND");
    public static final String ICE_AND_FIRE_SLIME = makeID("SlimeOfIceAndFire");
    public static final String METAL_LOUSES = makeID("Metal_Louses");
    public static final String CULTIST_ARMORER = makeID("Cultist_Armorer");
    public static final String ELEMENTAL_SENTRIES = makeID("Elemental_Sentries");
}
